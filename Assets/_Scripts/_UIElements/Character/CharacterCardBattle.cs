...
public class CharacterCardBattle : VisualElement
{
    VisualElement _information;
    CharacterStats _stats;
    public Character Character;
    protected CharacterPortraitElement _portrait;
    Label _name;
    ...
    public CharacterCardBattle(Character character, bool clickable = true)
    {
    ...
    public CharacterCardBattle(CharacterStats stats, bool clickable = true)
    {
    ...
    public List<ModifierElement> HandleStatuses(CharacterStats stats)
    {
        List<ModifierElement> els = new();
        if (stats.Statuses.Count == 0)
            return els;

        foreach (Status s in stats.Statuses)
        {
            ModifierElement mElement = new ModifierElement(s);
            els.Add(mElement);
        }
        return els;
    }

    public List<ModifierElement> HandleStatModifiers(CharacterStats stats)
    {
        List<ModifierElement> els = new();
        foreach (Stat s in stats.Stats)
        {
            ...
            {
                ModifierElement mElement = new ModifierElement(m);
                els.Add(mElement);
            }
        }
        return els;
    ...
    {
        ModifierElement mElement = new ModifierElement(mod);
        _portrait.Add(mElement);
    }

    void OnStatusAdded(Status status)
    {
        ModifierElement mElement = new ModifierElement(status);
        _portrait.Add(mElement);
    }

    void OnStatusRemoved(Status status)
    {
        ModifierElement elToRemove = null;
        foreach (ModifierElement el in _portrait.Children())
        {
